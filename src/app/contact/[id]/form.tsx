...
import {ContactSellerAction } from "@/lib/actions/cars.action";
import { ContactSellerSchema } from "@/lib/zod";
...
export const ContactSellerForm = ({ carId }: { carId: string }) => {
    ...
    const onSubmit = async (formData: ContactSellerSchema) => {
        ...
        try {
            setIsLoading(true)
            await ContactSellerAction(formData)
            toast.success("Message sent Successfully ")
            reset()
        } catch (error) {
            
            toast.error("Failed to sent Message .pleae try Again later ",error)
        }
