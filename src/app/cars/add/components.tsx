...
import { addNewCar, generatImage } from "@/lib/actions/cars.action"
...
import { autoGenerateCar } from "@/lib/actions/ai.actions"

export const GenerateImage = () => {

  const { register, formState: { errors }, handleSubmit } = useForm<GenerateImageSchema>({
    defaultValues: {
      description: "",
      name: "",
    },
  ...
  const onSubmit = async ({ description, name }: GenerateImageSchema) => {
    const toastId = toast.loading("Generate image")
    try {
      setGeneratingLoader(true)

      if (!description || !name) throw new Error("Descrition and Name Are Required")

      const data = await generatImage(description, name)

      if (!data) throw new Error("Error in generating image")
      setImage(data)
      ...
      toast.error("error generated image ", { id: toastId })
    }
    finally {
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground">
        Use AI generate an image of your dream car
      </p>
      <form onSubmit={handleSubmit(onSubmit)} className="mt-4 flex flex-col">
        <div className="space-y-2">
          ...
          <Textarea id="description"
            placeholder="Describe the car yiu want to generate .."
            rows={6}
            required
...
export const AddCarForm = () => {
  ...
  const { register, handleSubmit, formState: { errors }, setValue, watch, reset } = useForm<AddCarSchema>({
  ...
  const onsubmit = useCallback(
    async (data: AddCarSchema) => {
      ...
      try {
        ...
        else toast.error("error adding car listing")
  ...
  const autoGeneratehandler = useCallback(async () => {
    ...
    const toastid = toast.loading("Generating Car details ....")
    try {
